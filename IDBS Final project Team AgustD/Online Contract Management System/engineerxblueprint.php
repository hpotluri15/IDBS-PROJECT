...
if(isset($_GET['submit'])){
    ...
    $Tname=$_GET['TName'];
    if($Tname=="engineer"){
    $sql = " SELECT * FROM engineer JOIN blueprint ON blueprint.Engineer_Id=engineer.Engineer_Id WHERE blueprint.Engineer_Id= '$cond'" ;
...
if ($result->num_rows > 0) {
 ...
 if($Tname=="blueprint")
{
    $sql = " SELECT * FROM blueprint JOIN engineer ON blueprint.Engineer_Id=engineer.Engineer_Id WHERE engineer.Engineer_Id= '$cond'" ;
$result = mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
