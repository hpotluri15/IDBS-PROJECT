...
if(isset($_GET['submit'])){
    ...
    $Tname=$_GET['TName'];
    if($Tname=="blueprint"){
    $sql = " SELECT * FROM blueprint JOIN contractor ON contractor.Customer_ID=blueprint.Customer_Id WHERE contractor.Customer_ID= '$cond'" ;
...
if ($result->num_rows > 0) {
 ...
 if($Tname=="contractor")
{
    $sql = " SELECT * FROM contractor JOIN blueprint ON contractor.Customer_ID=blueprint.Customer_Id WHERE blueprint.Customer_Id= '$cond'" ;
$result = mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
